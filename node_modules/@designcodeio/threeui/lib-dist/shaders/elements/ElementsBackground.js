...
import { useRef as L, useState as x, useMemo as R, useCallback as z, useEffect as p } from "react";
import D from "./sources/elemental-marks.html.js";
const N = ["water", "lightning", "fire"], t = {
  variant: "water",
  speed: 1,
  size: 1,
  particleAmount: 1,
...
function O(e, a, o) {
  const n = U[e] / i(a, 0.65, 1.5), r = Math.max(0, Math.round(P[e] * i(o, 0, 2))), f = `<style data-elements-focus>
...
.panel[data-fx="${e}"] canvas { width: 100%; height: 100%; }
  ...
  return C(D).replace(/<link[^>]+fonts\.googleapis\.com[^>]*>/gi, "").replace(/<link[^>]+fonts\.gstatic\.com[^>]*>/gi, "").replace("</head>", `${f}<script data-elements-controls>
(function () {
...
<\/script></head>`).replace(new RegExp(`count: ${F[e]}`), `count: ${r}`).replace(`zoom: ${$[e]}`, `zoom: ${n.toFixed(4)}`).replace(
...
function Z({
  ...
  speed: a = t.speed,
  size: o = t.size,
  particleAmount: n = t.particleAmount,
  opacity: r = t.opacity,
  hue: f = t.hue,
  saturation: m = t.saturation,
  brightness: y = t.brightness,
  className: h = "",
  style: E
}) {
  const c = L(null), [M, k] = x(!0), [S, _] = x(() => typeof document > "u" || !document.hidden), l = N.includes(e) ? e : t.variant, u = i(a, 0, 3), v = !M || !S, b = R(
    () => O(l, o, n),
    [n, l, o]
  ), d = z(() => {
    c.current?.contentWindow?.postMessage({
      type: "elements-controls",
      controls: { speed: u, paused: v }
    }, "*");
  }, [v, u]);
  return p(() => {
    const s = c.current;
    if (!s || typeof IntersectionObserver > "u") return;
    const g = new IntersectionObserver(([A]) => k(A?.isIntersecting ?? !0));
    return g.observe(s), () => g.disconnect();
  }, []), p(() => {
    if (typeof document > "u") return;
    const s = () => _(!document.hidden);
    ...
    "div",
    {
      className: `threeui-background elements${h ? ` ${h}` : ""}`,
      "data-element": l,
      style: { background: "#060708", pointerEvents: "auto", ...E },
      children: /* @__PURE__ */ w(
        "iframe",
        {
          ref: c,
          title: `${T[l]} element background`,
          srcDoc: b,
          ...
          style: {
            ...
            background: "#060708",
            opacity: i(r, 0.05, 1),
            filter: `hue-rotate(${i(f, -180, 180)}deg) saturate(${i(m, 0, 2)}) brightness(${i(y, 0.35, 1.8)})`
          }
        }
      )
    }
...
export {
  t as ELEMENTS_DEFAULTS,
  N as ELEMENT_VARIANTS,
  Z as ElementsBackground
};
